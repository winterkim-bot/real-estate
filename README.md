...
지도의 최대 배율은 **배경과 화면에 따라 다릅니다**(`maxUsableZoom`). 뭉개지지 않는
마지막 배율까지만 확대되게 막습니다.

| 배경 | 일반 화면 | 고해상도 화면 |
| --- | --- | --- |
| Stadia (@2x 있음) | z20 | z20 |
| OpenStreetMap · 위성 | z19 | **z18** |

@2x가 없는 제공처는 '한 단계 높은 줌을 절반 크기로' 그려 밀도를 맞추는데, 타일이 있는
마지막 줌(z19)에서는 더 끌어올 게 없어 밀도가 1배로 떨어집니다. 3배 화면에서 1배는
픽셀이 사각형으로 뭉쳐 보이므로, 고해상도 화면에서는 한 단계 덜 확대되게 막았습니다.
어차피 같은 z19 타일을 보는 것이라 **보이는 정보는 같고**, 화면에 더 넓게·또렷하게 나옵니다.
